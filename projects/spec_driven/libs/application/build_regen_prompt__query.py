...
from libs.application.regen_prompt__qdto import RegenPromptQdto
...
class BuildRegenPromptQuery:
    ...
    def execute(
        ...
        revised_block, _ = self._inline_user_intent(project_type, project_name)
        ...
        if size > SOFT_LIMIT_BYTES:
            warning = {"kind": "approaching_ceiling", "bytes": size, "soft_limit": SOFT_LIMIT_BYTES}

        return RegenPromptQdto(
            prompt=prompt,
            warning=warning,
    ...
    def _render_stage_section(
        ...
        if promoted_path is not None and promoted_path.is_file():
            ...
            if body:
                ...
                chunks.append("")
                chunks.append(
                    f"_From `{promoted_rel}` — INPUT, not output. Insert verbatim at natural points; orphans go to a trailing `## Pinned items (orphaned)` section in the originally-pinned source file._"
                )
                chunks.append("")
                chunks.append(body)
                chunks.append("")
