 ...
 * its time window + desired emotion + author note. Per cue, the user assigns a
 * shared-library bgm_NNNN (picker filtered to the cue's category) — mirroring
 * casting. A single "🎵 烧录 BGM" burns the assigned cues onto the subtitled
      ...
      {data.cues.length === 0 ? (
        ...
        <table className="bgm-ep-table">
          ...
          <tbody>
            {data.cues.map((cue) => {
              const key = `${cue.start}-${cue.end}`;
              const opts = byCategory[cue.category] ?? [];
              ...
              return (
                <tr key={key} className={cue.assigned ? "bgm-ep-row-assigned" : "bgm-ep-row-open"}>
                    ...
                    <select value={cue.bgm_id ?? ""} disabled={busyWindow === key}
                      ...
                      {opts.map((b) => (
                        <option key={b.id} value={b.id}>
                          {b.id}{b.mood ? ` · ${b.mood}` : ""} ({b.duration}s)
                        </option>
                      ))}
                    </select>
                    {opts.length === 0 && <span className="bgm-ep-hint"> 库内无该类已生成 BGM</span>}
                  </td>
                  <td>
                    {assigned?.audio_path
                  ...
                  <td className="bgm-ep-params">
                    vol {fmt(cue.vol)}{cue.duck ? " · duck" : ""}
                    {cue.fade_in || cue.fade_out
                      ? ` · fade ${cue.fade_in ? "in" : ""}${cue.fade_in && cue.fade_out ? "/" : ""}${cue.fade_out ? "out" : ""}`
                      : ""}
                  </td>
