...
function extractDramas(tree: TreeNode | null): DramaChoice[] {
  if (!tree) return [];
  // Find ai_videos directly under root, then list non-`_*` immediate children.
  const aiVideos = findChild(tree, "ai_videos");
  if (!aiVideos) return [];
  const out: DramaChoice[] = [];
  for (const drama of aiVideos.children ?? []) {
    if (drama.type !== "directory") continue;
    if (drama.name.startsWith("_")) continue;
    const characters: string[] = [];
    const chDir = findChild(drama, "characters");
    if (chDir) {
...
function findChild(node: TreeNode, name: string): TreeNode | null {
