...
| K3 | **视频 prompt 字段齐全** | `## 视频 prompt` 代码块缺任一字段：参考 / 角色(+面部辨识特征) / 情节 / 场景 / 镜头 / 走位 / 动作 / 台词 / 光线 / 节奏 / 渲染样式 / 比例 / 时长 | 补缺字段（壳可空待审美 skill 填，但字段名必须在） | blocker |
...
| K8 | **锁定描述符 byte-identical** | shot `角色:` 行的"一句话锁定 / 面部辨识特征"与 `characters/{名}.md` 第 10 行锁定串不逐字相同（跨镜跨集漂移） | 用 bible 锁定串覆盖，byte-identical 重贴 | blocker |
| K9 | **零 hex** | 任何 prompt / 锁定 / 场景档 / style_guide 出现颜色 hex 码（#RRGGBB） | 删 hex，改自然中文色名（rule 12.8 / 12.3 / 12.4） | warning |
| K10 | **字数上限** | `## 视频 prompt` body > 2500 字 hard cap **且**无 12.4-E 例外注（cover-frame / ≥8s 全 enrichment） | 超且无注 → trim 或补 12.4-E 例外注；单/双角色 <8s 应在 2400 字内 | warning |
...
2. 逐 shot / 逐文件过 K1–K15，列出不合格项（cite 文件:行 / 准则 / 严重度）。
