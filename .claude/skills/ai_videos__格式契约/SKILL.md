...
| K22b | **多声轨 / 系统音色镜须视频静音 + 全 mux（2026-06-21 教训：EP3 shot7 同镜 系统×2(SYS-gold-01)+主角 OS(PR-hero-01)，靠 Seedance 单声轨 TTS → 语言彻底混乱、且用不上上传的系统 MP4 音色）** | 某 shot `台词:` 槽里写了 **≥2 个不同 voice_id 的发声单元**（系统+主角、甲+乙），或含**系统/合成锁定音色**句——指望 Seedance 一次出多个音色（必乱、且生不出系统锁定音色） | 改为视频静音 + 全 mux：`台词: 无` + 负面词加人声抑制组（同 K22），台词正文全移到 `## 台词配音 prompt` 各自 voice_id 块、后期 `tools/mux_av.py` 分轨 mux。⚠ 单声轨**在画口型对话**镜不适用本条（仍走 rule 704 保留 `台词:` 正文）；多声轨即便全不对口型也走本条 | blocker |
...
| K26 | **跨镜首帧承接落点三方一致（ai_video.md 2026-06-21 跨镜首帧承接）** | shot ① `## Shot context` 缺 `衔接:` 行（每镜必写 `承接 shot{NN} 末帧（首帧＝上一镜末帧）` 或 `硬切（独立首帧）` 其一；每集首镜须为硬切）；或 ② `衔接:`=承接，却缺三处落点任一——`参考:` 行最前的 `承接首帧(上一镜末帧)=>` handle / `Reference uploads` 的「上一镜末帧（从 shot{NN} …）」行；或 ③ `衔接:`=硬切，却残留 `承接首帧=>` handle / 末帧 upload 行（硬切镜禁挂承接帧） | 补 `衔接:` 行（承接/硬切判定本身归 `运镜` M8、本 skill 只校字段在场 + 落点齐全/互斥）；承接镜补齐 handle + upload 行；硬切镜删残留承接帧 handle/upload。承接 shot 号须指向相邻上一镜 | blocker |

## 工作流（校验 → 能定就改）
1. 读范围内全部目标文件 + 相关 characters/scenes bible（取锁定串 + voice_id）。
2. 逐 shot / 逐文件过 K1–K26（含 K22b 多声轨 mux、K26 跨镜首帧承接落点），列出不合格项（cite 文件:行 / 准则 / 严重度）。
