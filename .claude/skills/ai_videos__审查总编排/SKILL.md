...
| 1 | 单镜 | `ai_videos__台词大师` | 台词（说人话/白话/因果/信息量/声口/情绪/人称/节奏） | ✅ |
| 1b | 单镜 | `ai_videos__白话大师` | 白话口语化专项（朗读测试·去文言/对仗格言/翻译腔·书面词换口语）——紧跟台词大师，台词仍书面/不够口语时过 | ✅ |
| 2 | 单镜 | `ai_videos__站位朝向` | 站位/朝向/眼神/走位（A 别背对 B 说话、受责跪向锚定权威、内心独白视线、防角色重复） | ✅ |
| 3 | 单镜 | `ai_videos__运镜` | 景别/角度/镜头/运镜/180°轴线/视线匹配/9:16/相邻镜不重复 | ✅ |
| 4 | 单镜 | `ai_videos__动作表演` | 动作具体配时间轴/落钩不干站/配角反应/微表情/四层密度 | ✅ |
| 4b | 单镜(仅打斗) | `ai_videos__武打设计` | **打戏专项**：招式具体/攻防回合/受击打击感/战力分级/节奏镜头配合/藏锋无外放特效边界/平台合规——含打斗的 shot 才跑 | ✅ |
| 5 | 单镜 | `ai_videos__时长节奏` | 时长合理性裁决：台词念得完/动作演得开/不赶不拖（台词+动作 settled 后做时长仲裁） | ✅ |
| 6 | 单镜 | `ai_videos__光线色调` | 光线色调随情绪/场景一致/色彩锁定/回忆滤镜/眼不发光 | ✅ |
| 6b | 单镜(涉超自然视觉) | `ai_videos__特效设计` | **特效专项**：威能/法术/灵气/玉佩/暖流/系统UI/异象的有无·形态·藏锋外放边界·释放时机·氛围真实·平台合规——涉超自然视觉的 shot 才跑（与光线色调协作：发光禁令由光线色调落地、本 skill 定该不该有/边界） | ✅ |
| 7 | 整集 | `ai_videos__剧情连贯` | 情节链/动机/爽点落地/系统互动 + 相邻镜衔接 + 跨镜重复 + 本集专名时序 | ✅ |
| 8 | 全剧 | `ai_videos__全剧序列` | 跨集查重宣告/签名台词/开场结尾 + 爽点递进 + 跨集专名 + 锁定/voice_id 叙事一致 | ✅ |
| 9 | 整集+全剧 | 内建·立意安全 gate | **红果 2026-04-07 漫剧新规**：逐集查低俗擦边/血腥渲染/价值观倾斜；全剧查正向立意是否兑现、主角价值落点是否收束（见 ai_video.md §14.2）。无独立 skill，本编排内建执行 | ✅ |

> **第 9 步（立意+安全）落地方式**：整集层逐集自检有无低俗擦边/血腥/三观倾斜（命中=blocker，surgical 改或上报）；全剧层核对 concept.md 的「全剧正向立意 + 主角价值落点」是否在结局兑现。与 §nvdi020 的**生成块级**平台合规（敏感词/IP/武器）分工：那条管单镜 prompt 文字，本步管**整剧/整集叙事立意**。
