...
This file is **required pre-reading** for `agent_team__validation_manager` whenever the task being validated has `task_type=development`. It captures lessons learned across past development-task validations that the spec alone doesn't carry.

The validation manager MUST consult this file when defining the validation team for a development task. The parent (Claude in the spec_driven pipeline) is responsible for verifying that the manager actually loaded this file (check the manager's spawn audit `output.md` for explicit references to the rules below).
...
Numbered. Each is a level the validation team MUST include unless the spec explicitly excludes it.
...
Pure-shape unit tests (e.g., "the response has key X") are insufficient. The validation team MUST add at least one test per non-trivial API endpoint that **walks the response the same way the frontend code does**, descending into the same field names at the same depths.
...
**Concrete requirement:** the validation manager's strategy.md must list every API response shape with a "consumer field" column showing the exact path the frontend uses. If the backend test file references `result["projects"]` but the frontend reads `node.children`, that's a blocker, not a warning.
...
Development projects on this monorepo run on Windows + Git Bash. The validation team MUST mark these as `STATUS=SKIPPED-WINDOWS-...` rather than failing silently:
...
Even with Playwright, a final manual-walkthrough acceptance pass catches what e2e scripts miss: visual hierarchy, color contrast, motion, focus visibility under real input. The validation_manager (runtime mode) for development tasks MUST emit a `validation.requires_manual_walkthrough` event after all automated levels pass — and the parent must surface this to the user before declaring the unit done.
...
It IS a "what the validation manager has learned from prior development-task runs that the next spec author won't think to write down". Update it whenever a future run surfaces a class of bug that the existing strategy missed.
