...
> 7. **正常语速·超 15s 拆镜（时长节奏·编剧大师）**：台词按**正常语速**给足时长——**每镜「字数÷时长 ≤ 3.5」**（旧≤5 太赶、实测念不清）；念不完就加时长；**正常语速 > 15s 就拆成两镜**（不怕超时·每集总时长可放宽）。`ai_videos__时长节奏` 按 ≤3.5 裁决加时长/拆镜。
> 8. **生成/改 prompt 后默认自检·不待用户提醒（2026-06-21 用户实测：worker 重铺 shot14/15 超 2000 字未自查）**：**任何时候新建或修改一个生成 prompt（shot 视频prompt / 场景 seed·plate / 角色 ref / prop / 字卡），完成即默认自跑 `ai_videos__格式契约` 相关项——尤其 K10「```text 块 ≤2000 字硬顶」（shot 视频prompt 不计 `情节:` 块）**，超了当场精简（去重复藏锋句、压冗长走位/动作/光线、负向去重）再 declare done。**fan-out worker 产出回收后 parent 必须补跑这道自检**（worker 易写超长）。不依赖用户逐次提醒。
>
...
**⚠ 2026-06-21 amendment — 跨镜首帧承接（continuity-frame handoff）：强关联相邻镜，下一镜首帧＝上一镜成片末帧.** Per follow-up「两个 shot 之间若需强关联，要加收尾帧做指导——其实第二个 shot 的首帧就够；但不是所有 shot 都要（本就切镜头的不需要），缺了衔接会很不自然」。机制：
...
> 2. 承接镜 `## 视频 prompt` 的 `参考:` 行**最前**加 `承接首帧(上一镜末帧)=>` handle（标明它进模型「首帧」槽、非普通参考图槽）。
