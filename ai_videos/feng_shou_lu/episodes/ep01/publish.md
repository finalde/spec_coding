---
worker_id: parent-tier5
stage: 6
role: parent
work_unit_id: publish_pack-ep01
status: complete
blockers: []
confidence: high
---

# ep01 发布信息 · 《焚寿录》第一集 · 落雁渊

> 本文件供运营 / 剪辑后期 / 平台分发使用。所有内容中文 (per ai_video.md 规则 8 + spec FR-11). 字数限制: 标题 ≤ 30 字 / 简介 ≤ 200 字 / 标签 5–10 个. 封面建议指向 shotlist.md 中 `cover_frame: true` 的 shot07 lastframe 静帧 (per NFR-10).

---

## 标题（≤ 30 字, hook-first, 不剧透 cliffhanger）

**重生七岁孩童，胸前浮出寿元罗盘**

(28 字。备选两条，按平台 A/B 测试选用：
- **死过一回的人，还能再活几格寿？**（24 字, 痛点钩)
- **渡劫被剑刺穿，他在落雁渊渊底睁眼**（19 字, 反差钩)
)

## 简介（≤ 200 字, 设钩 + 暗扣 cliffhanger 不挑明）

化神剑修裴长砚，渡劫之夜被五个生死兄弟联手刺穿胸口；睁眼时，他成了澹江洲落雁渊渊底的七岁穷童。胸前浮出二十四格青铜罗盘——每升一阶吞他一年寿元、削他一段记忆。他用前世剩下的修真知识，水边划下新名「裴知秋」，决心在罗盘耗尽前手刃六个旧人。可这一夜，渊口苇草丛里站着的剪影，分明是早就死了的师父——师父怎么会在这里？

(190 字。痛点 + 反差 + 系统流 + cliffhanger 钩齐备, 不挑明「师父其实没死」/「主角即系统源头」等后期反转.)

## 标签（5–10 个, hashtag 风格, 平台 SEO）

- #仙侠短剧
- #重生
- #系统流
- #寄生系统
- #红果短剧
- #反派打脸
- #师父藏祸
- #寿元代价
- #AI短剧

(共 9 个; 抖音 / 红果 / 快手 短剧热门标签 + 本剧专属差异化 hook 字段.)

## 封面建议

**封面静帧来源**: `episodes/ep01/shots/shot07/shot07.md` lastframe (= `cover_frame: true` 标记帧, 满足封面安全区 contract per style_guide §11 + ai_video.md 规则 7).

**封面内容**: 全黑底 #0a0a0a + 居中楷体大字硬字幕「第二集 即将揭晓」(月白 #f5f5f0 + 寄生紫 #4a1a5a 描边, 48-56pt, y ≈ 600 黄金视觉中心) + 副位宋体小字金字「死过他的人——为何还活着？」(金字 #a8842c + 银边 #f5f5f0, 24-28pt, y ≈ 720).

**封面 1/4 缩略图可辨契约**: 主字幕「第二集」三字在抖音 feed / 红果信息流 1/4 缩略图尺寸下仍可清晰辨认 (style_guide §11 封面安全区 hard 契约).

**备选封面静帧 (A/B 测试)**: shot02 lastframe (戚归砚 内衬深紫一闪定格 + dual-affiliation 视觉伏笔), 或 shot05 lastframe (童子抬眼望渊口 + 胸前罗盘冷光晕未散 + 嘴角血一线). 但默认 cover_frame 锁定 shot07 cliffhanger 帧 (per NFR-10 cover_frame: true 强制).

## 平台备注

### 抖音 (Douyin)

- **post tag**: #仙侠短剧 #重生 #系统流 主，2 - 3 个标签限制内择优.
- **post 时间建议**: 晚 8 点 - 10 点（短剧黄金时段，男频仙侠用户活跃）. 周二 / 周四 / 周六首发，每周 3 更 (per dossier R-1 短剧爆款节奏).
- **特别说明**: 抖音封面安全区底部 240px + 左右各 60px + 顶部 200px 让位于平台 UI (账号头像 / 标题位 / 互动按钮). 主+副字幕已锁定 y ≈ 600 / y ≈ 720 黄金中心区, 不入封面安全区.

### 红果短剧 (Hongguo)

- **post tag**: #仙侠短剧 #重生 #红果短剧 主.
- **付费节点 1 提示**: ep10 是第一个付费节点 (卫长烛 face-slap + 失去母亲容貌记忆), 占整剧付费 ~70% (per dossier R-6 reveal cadence + arc_outline.md ep10 beat). ep01 - ep09 作免费引流段, ep10+ 起付费.
- **特别说明**: 红果对系统流 + 寿元代价 + 重生复仇 男频选题接受度高 (per trend_research §2.4 红果 2025 - 2026 头部仙侠选题分析).

### YouTube Shorts

- **post tag**: #xianxia #cdrama #shorts #fantasy 主 (英文标签 SEO).
- **EN 字幕 pass**: 留为 post-v1 follow-up (per spec § Out of scope #4). ep01 ship 中文版本; YouTube Shorts EN 版本 在 ep01 中文版本通过后另起 follow-up 翻译.
- **特别说明**: YouTube Shorts 算法对开头 3s 抓人能力极敏感 — 渡劫死亡 cold open (shot01 雷劫第三道 + 剑刺穿 + 闪黑) 是 YouTube Shorts 算法友好的开局.

---

*生成自 spec-driven 流水线 `xianxia_new-20260524-101931`. 本文件 review by user 后, 由运营 copy 进抖音 / 红果 / YouTube Shorts 后台发布表单.*
