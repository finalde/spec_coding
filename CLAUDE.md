...
- Every shot is **15 s** (default and target — was "≤ 15 s" before; bumped to fill Seedance's single-generation budget and give 镜头/运镜/动作/台词 more room). 15 s is the **target**, not just a ceiling — authors fill the full budget unless the scene genuinely doesn't warrant it (record a divergence note for any shorter shot). Anything longer than 15 s is two shots with a continuity token (rule unchanged). Per-Kling-cap (10 s) splits inside the same 15 s shot are an author-side concern handled with seam frames; the spec is always 15 s.
