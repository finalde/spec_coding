...
- **Render-side 台词烧录:** each shot may carry a `shots/shot{NN}/subtitles.md` dialogue-timeline (`起-止(秒) 台词文本` per line, OS/内心独白 unified bottom subtitle). The ai_video_management webapp burns it into a render via a per-mp4 button, producing `{stem}_subtitled.mp4` (original preserved). Decoupled from the in-prompt `台词 / 字幕:` field — see `.claude/agent_refs/project/ai_video.md` rule 11c.
