...
| FR-9f (actors/generate — first outbound HTTP) | SEC-OUTBOUND-KLING (see carve-out #7) | partial |
| FR-9g / FR-9h (casting assign/unassign) | SEC-CASTING-WRITE (see carve-out #7) | partial |
| FR-9i (actors/delete — soft-move + cascade unassign) | SEC-ACTORS-DELETE (see carve-out #7-bis) | partial |
| FR-86 (closed attribute schema) | SEC-OUTBOUND-KLING (input validation prevents prompt injection beyond known options) | covered |
| FR-9v / FR-9v2 / FR-9v3 (voices/generate — local composition, NO outbound HTTP) | SEC-VOICE-LOCAL-COMPOSITION (see carve-out #8) | partial |
| FR-9v5 (voices/{id}/audio — multipart upload) | SEC-VOICE-AUDIO-UPLOAD (see carve-out #8) | partial |
| FR-9v4 (voices/delete — soft-move + refuse-on-assignment) | SEC-VOICE-DELETE (see carve-out #8) | partial |
| FR-9v6 (casting/assign-voice / unassign-voice) | SEC-CASTING-WRITE (reuses actor carve-out #7 surface; voice_id column is the only delta) | partial |
| FR-86v (closed voice attribute schema) | SEC-VOICE-LOCAL-COMPOSITION (input validation prevents prompt injection beyond known options) | covered |
| FR-97 / FR-98 (prompt/suggest — second outbound HTTP, to Anthropic) | SEC-OUTBOUND-ANTHROPIC (see carve-out #9) | partial |
...
6. **`/api/import-from-downloads` reads outside EXPOSED_TREE** (follow-up 009). This is the FIRST endpoint to do so. Hardening (per FR-9e): (a) source dir is fixed to `Path.home() / "Downloads"` (env override available for testing only); (b) only immediate children — no recursive scan; (c) extension whitelist (MEDIA_EXTENSIONS); (d) mtime window ≤ 7 days; (e) symlinks refused; (f) basename validation via `_BASENAME_INVALID` regex + length ≤ 255 before any `shutil.move`. Destinations stay inside EXPOSED_TREE (drama folder). Residual risks: (i) a maliciously-named recent download could occupy a destination filename that collides with existing drama assets (mitigated by `target_exists` 409); (ii) Downloads dir is shared with the OS and may contain unrelated user files — if any happens to substring-match a folder name, it will be moved; files that match nothing are NOT imported (left in Downloads, reported as `unmatched`), so the blast radius is limited to substring collisions only. Confirm this residual surface is acceptable; if not, a future follow-up could add per-file user confirmation before move.

7. **`/api/actors/generate` makes outbound HTTP** (follow-up 014, evolved through 018 + 021 + 024 + 025 + 027 + 029). The backend issues requests to **Kling text-to-image** (`api.klingai.com/v1`). Follow-up 027 added (a) **race-safe ID allocation** — `_allocate_actor_id` uses `mkdir(exist_ok=False)` as the atomic primitive so the frontend's 9-worker concurrent pool cannot land two actors at the same `actor_NNNN/` folder; the prior pre-computed `next_id + offset` pattern would have raced with itself once concurrency was introduced. (b) **Server-side variance pools** — `_variance_for(seed, gender)` selects fragments from five hardcoded English tuples (face features / skin / face shape / eyes / hair) seeded by the per-image seed; **the variance string is never user-controlled**, closing off any per-image prompt-injection surface that a free-form variance field would have opened. Follow-up 029 expanded the variance to **17 hardcoded pools** producing ≥ 1000 chars per prompt — same hardening (server-side only, seed-deterministic, no user input enters the variance string). (c) **Batch count cap raised 20 → 50** — still bounds the total outbound HTTP wave per request; combined with `CONCURRENCY=9` worker pool the worst-case in-flight set is 9, matching Kling's documented limit. Follow-up 029 also added: (d) **Pillow image-decode + resize step** for resolutions `2k` / `4k`. The bytes Pillow decodes come from a single trusted source — the Kling-returned CDN URL that was already SSRF-vetted (h) and size-capped at 5 MiB before reaching Pillow; Pillow is not used on arbitrary user uploads. JPEG decoding only (RGB conversion is explicit via `.convert("RGB")`). Pillow CVEs that target malicious PNG / SVG / TIFF / multi-frame formats are not in scope because the only inputs are Kling's JPEG outputs; if Pillow throws on a malformed payload, the failure is caught and recorded as `resize_failed` in `errors[]` (the batch continues). The new `pillow>=10.0` dependency pulls in libjpeg etc. — track upstream Pillow security advisories on subsequent rebases. (e) **Resolution enum** `{"normal", "2k", "4k"}` is closed and validated server-side via `InvalidAttribute`; the only filesystem effect is the JPEG file's pixel dimensions and on-disk size (4K JPEG ≈ 1–3 MB per actor; a 50-actor 4K batch ≈ 50–150 MB — user-managed disk budget). Follow-up 025 collapsed the prior multi-provider chain (Pollinations + AI Horde + Kling) to Kling-only: Kling's ~1-3s/img + JWT auth + commercial stability rendered the chain's failover insurance moot while keeping three outbound hosts + a public anonymous AI Horde apikey + an r2.dev TOCTOU surface alive for no benefit. **Credentials**: `KLING_ACCESS_KEY` + `KLING_SECRET_KEY` are required; loaded at startup by `libs.env_loader.load_env_file()` from `projects/ai_video_management/backend/.env` (the file is gitignored at repo root by the `.env` pattern; the loader uses `os.environ.setdefault` so an already-set shell / CI env wins; missing keys raise `RuntimeError` at `create_app()` time — startup failfast). Kling auth: JWT HS256 with stdlib `hmac`+`hashlib`+`base64`+`json` (no PyJWT dep); claims `{iss: AK, exp: now+1800, nbf: now-5}`; signed per-request (30-min validity, no in-memory cache so a leaked stale token can't be reused after exp). Hardening: (a) base URL `https://api.klingai.com/v1` hardcoded; the user-controlled portion is the prompt text only, assembled server-side from the closed enum schema (FR-86); (b) per-fetch 30s timeout; (c) per-fetch 5 MiB response cap; (d) batch size cap 20 — bounds total wall-clock; (e) submit response `code != 0` is explicitly checked even on HTTP 200 to surface application-level errors instead of swallowing them; poll loop has 120s wall-clock cap then `TimeoutError`; (f) **`KLING_SECRET_KEY` is a real secret** — read only from env, fed only into `hmac.new`, never logged, never embedded in URLs, never returned in JSON responses; `KLING_ACCESS_KEY` is placed in the JWT `iss` claim per Kling's spec (Kling treats AK as identifier, not secret); the `.env` file is outside EXPOSED_TREE (`projects/` is not in the 5 allowed roots — FR-7), so `GET /api/file?path=...` cannot leak it; (g) the CDN URL returned by Kling is **SSRF-vetted** by resolving its hostname and rejecting any loopback / RFC1918 / link-local / multicast / reserved IP; download forces https scheme; only after vetting is the URL fetched with `follow_redirects=True` (Kling's r2-class CDN URL typically redirects); (h) writes land only inside `ai_videos/_actors/actor_NNNN/` (EXPOSED_TREE-inside; ID monotonically allocated; incomplete folders reaped per follow-up 018). Residual risks: (i) Kling availability is a hard dependency (no fallback chain post-025) — total Kling outage manifests as 100% `errors[]` per batch (acceptable; user retries later); (ii) Kling's CDN host is controlled by its servers — a compromised Kling could attempt to redirect to a non-IP-listed hostname that resolves later to a private address (TOCTOU); SSRF guard runs immediately before fetch, `getaddrinfo` is used for resolution (browser-class threat model); residual TOCTOU window is sub-millisecond, acceptable for localhost-only tool; (iii) generated images NOT content-filtered — degenerate prompts may yield undesirable output; user must delete via the per-file delete UI (follow-up 023) or filesystem; (iv) backend has internet egress on user request — anyone with localhost shell access can trigger external requests at our IP (low risk on personal laptop; material if exposed to multi-user). The CSP `connect-src 'self'` is unchanged: the frontend never directly contacts Kling; all generation flows through `POST /api/actors/generate`. **`/api/casting/assign` writes** only touch `ai_videos/{drama}/casting.md` inside EXPOSED_TREE; the same Origin/Host gap exists as for rename/archive/import (not in `GUARDED_ROUTES`) — confirm whether widening the gate is in or out of scope before merge.

7-bis. **`/api/actors/delete` soft-moves an actor folder + cascades casting unassign** (follow-up 026). New write surface added on top of carve-out #7's `actors/generate` posture. Hardening (per FR-9i): (a) `actor_id` shape strictly `^actor_\d{4,}$` — no path-separators or traversal characters reach the filesystem; (b) source path is derived from `actors_dir() / actor_id` (no user-supplied path), so the source is provably inside `ai_videos/_actors/`; (c) target path is fixed-derived `ai_videos/_deleted/_actors/{actor_id}/` — also provably inside EXPOSED_TREE; (d) symlinks refused (`is_symlink()`); (e) atomic `Path.rename` — no partial state visible to other callers; (f) cascade sweep walks ONLY immediate children of `ai_videos/` whose name does NOT start with `_` (system folders excluded), reads only `casting.md` files, writes via the same temp+`os.replace` atomic path used by `POST /api/casting/assign` (FR-9g); the cascade does NOT touch any other file; (g) cascade-first ordering means a mid-sweep OS error leaves the actor in place (idempotent retry safe). Residual risks: (i) the endpoint is NOT in `GUARDED_ROUTES` — same Origin/Host gap as rename/archive/import/delete-media/actors/generate (decision deferred to a follow-up that widens the gate for all of them at once; not a new exposure relative to carve-out #7); (ii) the `actor_NNNN` ID slot becomes available for reuse by a subsequent `POST /api/actors/generate` because `_next_actor_id_num` does NOT scan `_deleted/_actors/` — this is intentional (deletion semantics) but means a future restore-from-`_deleted/` workflow would have to detect ID collisions; out of scope for v1 (no in-app restore); (iii) the cascade rewrites every drama's `casting.md` that referenced the deleted actor, mutating multiple files in a non-atomic sequence — interleaved external edits (manual `casting.md` save through `PUT /api/file` mid-cascade) could be overwritten; race window is sub-second for a typical drama set and acceptable for a localhost single-user tool. Confirm.

8. **Voice pool surfaces (follow-up 115).** Three new write paths land in U8; security posture is materially smaller than the actor pool because the pipeline is **local-only by design**.
   - **SEC-VOICE-LOCAL-COMPOSITION** (FR-9v / FR-9v2 / FR-9v3). Voice prompt composition is fully local — no outbound HTTP, no provider credentials, no SSRF surface, no JWT, no 429 retry, no `_validate_outbound_url`. The variance pools (parallel to actor's `_LOOK_FEATURE_BIAS_ZH`) live server-side and are never user-controlled; user input is restricted to the closed FR-86v enum + a free-form Chinese `tone` / `signature_inflection` / `notes` field. These free-form fields are written verbatim into the sidecar `voice_NNNN.md` (markdown context) — markdown sanitization (FR-16 / SEC-SANITIZE) already covers the rendering surface. Hardening parity with actor pool: (a) race-safe ID allocation via `mkdir(exist_ok=False)` walking forward from `_next_voice_id_num()`; (b) `_reap_incomplete_folders()` sweep at batch top drops md-less folders left by killed batches; (c) writes land only inside `ai_videos/_voices/voice_NNNN/` (EXPOSED_TREE-inside); (d) batch count cap 50 matches actor pool. **Stage-6 validators must grep-fail** if any of `httpx.AsyncClient`, `urllib.request`, `requests.`, a literal `https://` URL string, or a new file under `libs/infrastructure/clients/voice_*.py` appears anywhere in the U8 deliverables — this is the load-bearing assertion that keeps the voice pool's threat model small.
   - **SEC-VOICE-AUDIO-UPLOAD** (FR-9v5). NEW write surface beyond `PUT /api/file`. Hardening: (a) extension allowlist enforced server-side on the multipart filename → `.mp3` / `.wav` / `.m4a` only; non-allowed → `400 extension_not_allowed` BEFORE bytes touch disk; (b) **10 MiB body cap** (audio is larger than markdown; explicitly NOT reusing the 1 MiB cap from FR-14); >10 MiB → `413 body_too_large`; (c) destination path is fixed-derived `ai_videos/_voices/{voice_id}/{voice_id}.{ext}` — user supplies no path component beyond the `voice_id` (validated `^voice_\d{4,}$`); (d) target path verified inside EXPOSED_TREE before write; (e) symlinks at target refused; (f) overwrites prior sample atomically via temp+`os.replace`; (g) sidecar's `audio_sample` row is updated atomically AFTER successful audio write so a write-then-crash leaves a consistent sidecar pointer; (h) Origin/Host gate applies (this endpoint MUST be added to `GUARDED_ROUTES`). Residual risks: (i) malformed audio bytes are NOT decoded server-side (no Pillow-equivalent decode step) — the file is stored as-is and played by the browser's native `<audio>` element, so browser-side codec CVEs apply (acceptable for localhost-only tool with user-supplied samples); (ii) audio metadata (ID3, RIFF) may contain crafted strings — never parsed by the backend, only by the browser audio renderer; (iii) no virus scan (out of scope; same posture as actor JPEGs).
   - **SEC-VOICE-DELETE** (FR-9v4). Mirrors SEC-ACTORS-DELETE (carve-out #7-bis) with one improvement: voice deletion **refuses-on-assignment** instead of cascade-unassigning (matches the actor-side amendment from follow-up 043). `voice_id` shape `^voice_\d{4,}$`, source derived from `voices_dir() / voice_id`, target fixed-derived `ai_videos/_deleted/_voices/{voice_id}/`, symlinks refused, atomic `Path.rename`. The refuse-on-assignment posture eliminates the multi-file non-atomic cascade race that carve-out #7-bis residual-risk (iii) noted.
   - **Multipart parsing dependency.** FR-9v5 is the first endpoint to use FastAPI's `UploadFile` / `python-multipart` parser. Track upstream `python-multipart` CVEs on rebase; pin minimum version per project requirements file.

9. **`/api/prompt/suggest` makes outbound HTTP to Anthropic** (follow-up 117). The SECOND outbound-HTTP endpoint after Kling (carve-out #7). The backend calls the Anthropic Messages API via the official `anthropic` SDK (over the SDK's own https client; base URL is the SDK default). Posture: (a) **read-only / suggestion-only** — the endpoint never writes any file; merging a chosen suggestion into a prompt + saving goes through the existing `PUT /api/file` surface (FR-15) under explicit user confirmation, so this endpoint adds no new write surface; (b) **credentials** `ANTHROPIC_API_KEY` (+ optional `ANTHROPIC_SUGGEST_MODEL`) are read only from env (loaded from `apps/api/.env` by the same `libs.common.env_loader` used for Kling, outside EXPOSED_TREE so `GET /api/file` cannot leak them); missing key → `503 suggestion_unavailable` (no startup failfast — the feature degrades gracefully, unlike Kling which is required); the key is fed only to the SDK constructor, never logged, never returned in responses; (c) **request content** — the shot context + dimension + current prompt body are sent to Anthropic as the user message. This is the user's own shot-planning content leaving the machine to a third-party LLM; it is the explicit purpose of the feature, but note that enabling the key opts every ✨ click into transmitting shot text to Anthropic. Confirm this data-egress is acceptable; with no key the feature is inert and nothing is transmitted. (d) the SDK is imported lazily and the client built lazily, so neither module import nor `create_app()` fails when the SDK is absent or the key is unset; SDK / API failures are caught and surfaced as `502 suggestion_failed`; (e) response parsing extracts a JSON array defensively (strips code fences, locates the bracketed array) and drops malformed items, so a non-conforming model reply yields `502` rather than propagating arbitrary text. Residual risks: (i) backend has internet egress to Anthropic on user request — same localhost-shell-access consideration as carve-out #7 (iv); (ii) no rate limit on `/api/prompt/suggest` — a UI loop could run up Anthropic token cost; bounded in practice by the manual click-per-dimension UX (out of scope for v1, same posture as carve-out #5); (iii) `connect-src 'self'` CSP is unchanged — the frontend never contacts Anthropic directly; all calls flow through `POST /api/prompt/suggest`. The endpoint is suggestion-only and not in `GUARDED_ROUTES` by default — same Origin/Host gap noted for the other POST endpoints (decision deferred to the gate-widening follow-up).
