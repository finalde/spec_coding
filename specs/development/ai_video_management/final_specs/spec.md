...
- **FR-9j** (follow-up 032) `POST /api/actors/preview-prompts` — dry-run prompt computation. Body matches `GenerateActorsBody` (attrs + count + resolution; `seeds` field is ignored). Backend's `ActorPool.preview_prompts` validates attrs / count / resolution (same errors as FR-9f), computes a fresh `base_seed = int(time.time() * 1000)` and produces `{prompts: [{seed, prompt}], resolution}` — one entry per slot containing the EXACT prompt that `generate_batch` would send to Kling for that seed. NO files are written, NO Kling call is made, NO actor folders are allocated. Status: `200`, `400 invalid_attribute`, `405 method_not_allowed`. The frontend uses this to render a "review before send" modal; on confirm it calls `POST /api/actors/generate` with `seeds: [...]` set to the previewed seeds so the actual Kling submission is byte-equal to what the user reviewed.

- **FR-9i** (follow-up 026) `POST /api/actors/delete` — soft-delete an actor folder and cascade-unassign every casting reference. Body `{actor_id: "actor_NNNN"}` (shape `^actor_\d{4,}$`). Backend (1) walks every immediate non-`_`-prefixed child of `ai_videos/`, parses each drama's `casting.md` (if present), removes every row whose `actor_id` matches, and atomically rewrites the file via the same temp+`os.replace` path used by `POST /api/casting/assign`; (2) then atomically `Path.rename`s `ai_videos/_actors/actor_NNNN/` → `ai_videos/_deleted/_actors/actor_NNNN/`, mkdir-ing `_deleted/_actors/` parent on demand. Cascade-first ordering: if the cascade fails mid-sweep the folder is NOT moved, so retry is idempotent (subsequent cascade finds nothing to remove). Response: `{from, to, unassigned: [{drama, role}]}`. Status codes: `200`, `400 invalid_actor_id` (shape mismatch), `404 actor_not_found` (folder missing / symlink), `409 target_exists` (target path already populated), `500 cascade_failed` (OS error during casting sweep), `500 move_failed` (OS error during rename / mkdir), `405 method_not_allowed`. Symlinks are refused (`is_symlink()` check). The actor-id allocator (`_next_actor_id_num`) only scans `_actors/`, NOT `_deleted/_actors/`, so deleted IDs become available for reuse — this is intentional v1 behavior; an in-app restore is out of scope. Image extension allowlist / EXPOSED_TREE membership is unchanged (target stays inside `ai_videos/`).
...
- **FR-87** `_actors` is NOT a drama. `sub_type_lookup` returns `None` for it (no `episodes/`, no `script.md`/`shotlist.md`), the sidebar emits no sub-type badge for it, and the drama-row "📥 导入 + 重命名" button is NOT rendered on the `_actors/` row. Instead, the sidebar emits a "🎭 生成演员" button on the `_actors/` row which opens the `ActorPoolGenerator` modal, and (per follow-up 028) a sibling "🔲 网格" button that navigates to the `/actors` route rendering `ActorGrid` (see FR-91). **Per follow-up 026**: every `ai_videos/_actors/actor_NNNN/` sub-row also renders a "🗑" delete button that calls `POST /api/actors/delete` (FR-9i) after a `window.confirm` and surfaces the `{unassigned: [...]}` count in the sidebar toast. The button is hidden on actor folders that already live under `_deleted/` (path-prefix check) so users cannot trigger nested `_deleted/_deleted/`. Convention: any `ai_videos/{x}/` folder whose name starts with `_` is treated as a system folder (not a drama, no rename button, no sub-type detection).
