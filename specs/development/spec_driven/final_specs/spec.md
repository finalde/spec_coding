...
**FR-15.** Recursive sidebar walks `node.children` uniformly (no `task_type.projects` / `project.stages` branches). Tree section names: "Claude Settings & Shared Context" + "Specs". The `Specs/{type}/{name}/` subtree shows per-stage subfolders (`user_input/`, `interview/`, `findings/`, `final_specs/`, `validation/`). When an active project is set (FR-42), the Specs section is filtered client-side to ONLY the active `{type}/{name}` subtree — every other project under `specs/` is hidden from the sidebar. When no active project is set, the Specs section's children are an empty list. The Claude Settings & Shared Context section is unchanged in either mode (cross-project context, never filtered). The wire shape from `GET /api/tree` is unchanged; filtering is purely a frontend concern. Both top-level sections are collapsible — clicking the disclosure caret or pressing Enter/Space on the section row toggles its open/closed state, ArrowRight expands a collapsed section, ArrowLeft collapses an expanded section. Default on first render is expanded; collapse state is component-local React state, not persisted to `localStorage` in v1.
...
**FR-42.** Landing project picker at route `/`. Renders WITHOUT the sidebar — full-width main pane. Lists every `{task_type, task_name}` pair derived from the tree's "Specs" section's first two child levels (e.g., `development/spec_driven`, `development/ai_video_management`, `ai_video/<name>`). Each row is a clickable link. Clicking a project (a) sets `activeProject = "{type}/{name}"` in `localStorage` under the key `spec_driven.active_project.v1`, and (b) navigates to `/project/{type}/{name}` (FR-32). On any non-landing route the sidebar mounts and a "← Back to projects" link in the sidebar header (above the tree, NOT a treeitem so it does not interfere with the FR-18 keyboard contract) clears `activeProject` and navigates back to `/`. Active-project state is also re-derived from URL parameters whenever a project-scoped route loads directly: `/project/:projectType/:projectName`, `/stage/:projectType/:projectName/:stage`, and `/file/specs/{type}/{name}/...` all repopulate `activeProject` so deep-links and bookmarks survive a fresh browser. Routes that do not name a project (`/file/CLAUDE.md`, `/file/.claude/...`) read `activeProject` from `localStorage`; if unset, the sidebar still renders but the Specs section is empty. (Added by follow-up 012; supersedes the prior behavior where `/` rendered both the sidebar and an inline project list.)

### Prompt Lab (added by follow-up 016)

**FR-43.** A **Prompt Lab** surface exposes the repo-root `prompt_lab/` library — category subfolders (`prompt_lab/{category}/{task}.md`) of copy-paste, autonomous "Ralph-loop" build prompts — as a top-level section parallel to Specs. It does NOT require an active spec project.

- **Sandbox.** `prompt_lab/` is added as an allowed top-level root in `safe_resolve` and `exposed_tree.is_inside`, so the generic `GET/PUT /api/file` serve read + update + create-in-existing-category for `prompt_lab/*.md`. The generic `DELETE /api/file` stays 405 (verb whitelist unchanged); file deletion is exposed only through the `prompt_lab/`-scoped endpoint below. `specs/`/`projects/` deletion is NOT opened up.
- **Overview.** `GET /api/prompt-lab` returns `{categories: [{name, entries: [{path, name, title, meta, source, expected, prompt}]}]}`, parsed from each md: `title` = first H1; `meta` = the `Stack / Est / Output` line; `source` / `expected` = the `**Source:**` / `**Expected result:**` link lines; `prompt` = the first fenced code block (the copy-paste prompt).
- **Create.** `POST /api/prompt-lab/file` `{category, filename, content}` ensures the category dir under `prompt_lab/`, refuses overwrite (409), validates the slug + `.md` extension, and writes atomically. Returns the write result.
- **Delete.** `DELETE /api/prompt-lab/file` `{path}` deletes a single `.md` whose path resolves inside `prompt_lab/`. 404 if absent.
- **UI.** Route `/prompt-lab`, reachable from the landing page. Rendered as the app's standard two-column `app-root` layout (amended by follow-up 017): a left `.sidebar` lists categories → task items (with a "← Home" link and a "+ New" button); the `.main-pane` shows the selected task. Viewing/editing is **inline within this page** (amended by follow-up 019): selecting a task fetches its content (`GET /api/file`) and renders it with `Renderer` (highlighted prompt block + Copy button + click-to-edit preserved); a detail toolbar offers ✎ Edit (swaps to the inline `Editor`, saving via `PUT /api/file` with the `If-Unmodified-Since` 409 guard) and Delete (confirmed); "+ New" opens the inline editor on the created file. The page never navigates to `/file/{path}` (doing so previously swapped in the spec project's sidebar — the bug 019 fixes); the global `/file/prompt_lab/...` route still works for deep-links. *(Superseded the original follow-up-016 full-width tile grid.)*
- **Prompt highlight + click-to-edit + copy (amended by follow-up 018).** When the reader renders a `prompt_lab/` md, the copy-paste prompt code block renders highlighted (primary artifact) with a **Copy** button (copies the prompt text; `stopPropagation` so it doesn't also trigger edit), and clicking the block switches the file into edit mode (the existing whole-file editor). The Prompt Lab detail pane carries the same Copy action.
- **Fixed 3-section task structure (added by follow-up 018).** Every `prompt_lab/` task `.md` follows the order: title + `Stack/Est/Output` meta, then `✨ 1. Expectation — what you'll get`, `▶️ 2. How to run`, `🔗 3. Source & references` (carrying the `**Source:**` + `**Expected result:**` links), then the `## 📋 COPY-PASTE PROMPT` block. The webapp's new-prompt template seeds this structure; the overview parser is unchanged (keys on the `**Stack:**` / `**Source:**` / `**Expected result:**` lines + first fenced block).

### Prompt Lab autonomous execution (added by follow-up 020)

**FR-44.** Each Prompt Lab item is its own workspace folder `prompt_lab/{category}/{task}/` containing `prompt.md`; the overview parser walks `{category}/*/prompt.md`, create makes `{cat}/{slug}/prompt.md`, delete removes the whole item folder.

- **Execute.** `POST /api/prompt-lab/execute {path}` spawns `claude --print --permission-mode bypassPermissions` as a background process with cwd = the item's `workspace/`, fed (stdin) the item's copy-paste prompt prefixed with a `# EXECUTION MODE: AUTONOMOUS` header + an autonomy contract: never wait for input; for any setup question / choice / ambiguity pick the most sensible default and proceed; log every decision as one JSON line (`{ts, question, decision, why}`) to `decisions.jsonl`; build to completion. Returns `{state, run_id, path}`. 409 if a run is already in progress; 503 if the `claude` CLI is unavailable. Strictly `prompt_lab/`-scoped.
- **Run state.** `GET /api/prompt-lab/run?path=...` → `{state (idle|running|succeeded|failed|stopped), run_id, started_at, ended_at, exit_code, output, decisions[], files[]}`. Run state persists in `status.json`; pid-liveness reconciles a stale "running".
- **Stop.** `POST /api/prompt-lab/stop {path}` best-effort kills the process tree.
- **UI portal.** The item detail has a ▶ Execute / ⏹ Stop button and a run panel that polls while running and shows the status badge, the **autonomous decisions** the session made (question → decision + why), the live session output, and the generated files. The overview also carries a per-item `run_state`.
