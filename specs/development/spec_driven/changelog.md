...
## Follow-up 015 — 2026-05-17 18:00:24
Source: user_input/follow_ups/015-20260517-180024-evolved-libs-layout-standard.md
Summary: Project adopts the **evolved** form of the `apps/+libs/` + DDD + CQRS standard that `.claude/agent_refs/project/development.md` accumulated through ai_video_management follow-ups 056 / 060 / 061 / 065 / 068. Per-role sub-bucketing within each layer (`infrastructure/{readers,writers,clients,daos,middleware,errors}/`, `domain/{entities,value_objects,errors,repositories}/`, `application/{queries,commands,dtos,mappers}/`); one file per aggregate per role (`{aggregate}__{role}.py`); ONE class per file in `commands/` + `queries/` with method-per-operation (`PromotionCommand.add(...)` + `.remove(...)`, not `AddPromotionCommand.execute()`); DTOs consolidated into one `dtos/{aggregate}__dto.py` per aggregate; routes split per aggregate at `apps/api/routes/{aggregate}__route.py` each with its own `APIRouter()`, combined via `routes/__init__.py`; SRP — infra exceptions extracted to `libs/infrastructure/errors/{aggregate}__error.py`, DAO dataclasses to `daos/{aggregate}__dao.py`, Pydantic request bodies stay with the route handler; `< 100` line file-size guideline, `> 1000` lines `warning`. HTTP routes + JSON shapes byte-identical to v1 — only internal organization changes.

Auto-updated:
- specs/development/spec_driven/user_input/revised_prompt.md — `Last regenerated` header bumped to 2026-05-17 18:00:24 with follow-up 015 noted; Stack-section "Layout" bullet amended to describe the role sub-buckets, the one-file-per-aggregate rule, and the routes-package split.
- specs/development/spec_driven/final_specs/spec.md — second amendment block (follow-up 015) stacked under the existing 014 block, restating the seven evolved rules verbatim and pointing NFR-4's stale `backend/libs/` reference at `libs/{infrastructure,domain,application,common}/`.
- specs/development/spec_driven/validation/strategy.md — second amendment block (follow-up 015) added under the existing 014 block, introducing three new stage-5 `blocker` checks (sub-bucket presence; SRP infra-exception grep; routes-split presence) and a new stage-5 `warning` for the > 1000-line file-size guideline.

No conflicts found in: interview/qa.md, findings/dossier.md, final_specs/promoted.md, validation/{acceptance_criteria,bdd_scenarios,system_tests,unit_tests,security,performance,accessibility}.md (covered by the strategy.md umbrella amendment), validation/promoted.md, historical follow-ups 001–014 (immutable history; preserved).

Out-of-band note (not auto-fixed in this follow-up — staged for a future stage-6 regen of `projects/spec_driven/`): the current code under `projects/spec_driven/libs/` is still on the **older** flat-per-layer + per-operation-file form from follow-up 014 (e.g., `libs/infrastructure/file__writer.py` at the layer root instead of `libs/infrastructure/writers/file__writer.py`; `libs/application/add_promotion__command.py` + `remove_promotion__command.py` instead of `libs/application/commands/promotion__command.py` with `.add(...)` + `.remove(...)`; likely a single `apps/api/routes/` file instead of per-aggregate `*__route.py`; infra exceptions still defined inline in writer/reader files). Bringing the code in line is a stage-6 task — this follow-up persists intent only.
...
Auto-updated:
- specs/development/spec_driven/final_specs/spec.md — FR-15 amended (top-level sections collapsible; default expanded; component-local state, not persisted).
- specs/development/spec_driven/user_input/revised_prompt.md — `Last regenerated` header bumped to 2026-05-09 12:01:57 with follow-up 013 noted; body unchanged (intent does not move).
- projects/spec_driven/frontend/src/components/Sidebar.tsx — new `nodeKey(node)` helper; the depth-0-always-open special case in `flat` removed (uniform `expanded[nodeKey(node)] === true`); pre-seed `useEffect` now seeds sections too via `nodeKey(node)`; click + disclosure + keyboard handlers (`toggle`, `onItemKeyDown`, `focusByPath`) keyed by `nodeKey(item.node)`; treeitem `data-path={itemKey}` and `key={itemKey}` use the same key (disambiguates the two empty-path sections); new `tree-section` className applied when `node.type === "section"` for visual treatment.
- projects/spec_driven/frontend/src/styles.css — new `.tree-item.tree-section` rule (uppercase 11px section-label styling).

No conflicts found in: interview/qa.md, findings/dossier.md, validation/* (the 3 cases in `test/Sidebar.test.tsx` make assertions on leaf-name visibility — default state is expanded, so leaves remain visible and all 56 vitest cases still pass), projects/spec_driven/backend/* (no backend changes — tree wire shape and section paths unchanged), projects/spec_driven/frontend/src/{App.tsx, components/Home.tsx, components/Reader.tsx, components/ProjectPage.tsx, components/StagePage.tsx, lib/activeProject.ts} (untouched — they don't depend on sidebar collapse state).

## Follow-up 012 — 2026-05-09 11:47:28
Source: user_input/follow_ups/012-20260509-114728-landing-page-and-scoped-sidebar.md
Summary: Landing project picker + scoped sidebar. The webapp's `/` route now renders a clean project picker WITHOUT the left sidebar (full-width main pane); clicking a project sets `activeProject` in `localStorage` (key `spec_driven.active_project.v1`) and navigates to `/project/{type}/{name}`. On every non-landing route the sidebar mounts, and the **Specs** section is filtered client-side to ONLY the active `{type}/{name}` subtree (every other project under `specs/` is hidden); the **Claude Settings & Shared Context** section stays unchanged. A new "← Back to projects" link in the sidebar header (above the tree, NOT a treeitem — preserves the FR-18 keyboard contract) clears `activeProject` and returns to `/`. Active-project state is also re-derived from URL params for `/project/:projectType/:projectName`, `/stage/:projectType/:projectName/:stage`, and `/file/specs/{type}/{name}/...` so deep-links / bookmarks survive a fresh browser. Filtering is purely a frontend concern; the backend `/api/tree` wire shape and `EXPOSED_TREE` path-sandbox are unchanged.

Auto-updated:
- specs/development/spec_driven/final_specs/spec.md — FR-15 amended (Specs section filtered to active project; empty when none); new FR-42 (landing picker route + activeProject localStorage key + Back-to-projects link + URL re-derivation contract).
- specs/development/spec_driven/user_input/revised_prompt.md — `Last regenerated` header bumped to 2026-05-09 11:47:28 with follow-up 012 noted; "What the viewer/editor surfaces" section split into two layout modes (landing-mode picker / workspace-mode sidebar).
- projects/spec_driven/frontend/src/lib/activeProject.ts — new module. Wraps `TypedLocalStorage<string|null>` under `spec_driven.active_project.v1`; exports `ActiveProject` type, `parseActiveProject`, `readActiveProject`, `writeActiveProject`, `activeProjectStorageKey`.
- projects/spec_driven/frontend/src/App.tsx — tracks `activeProject` state; new `deriveActiveProjectFromPath` helper syncs activeProject from URL on every route change. Layout split: when `location.pathname === "/"`, render `<Home …/>` only inside `<main className="picker-pane">` with a new `app-root-landing` modifier class on `.app-root` (no sidebar, no two-column grid). On non-landing routes, render the existing two-column layout but pass `activeProject` and `onBackToProjects` to `<Sidebar>`. `Routes` no longer includes `<Route path="/" />` since `/` is handled outside.
- projects/spec_driven/frontend/src/components/Sidebar.tsx — new required props `activeProject: ActiveProject | null` and `onBackToProjects: () => void`. New `filterSpecsForActiveProject` prunes the Specs section to only the active `{type}/{name}` subtree (or empty children when null). New `<div className="sidebar-header">` above the tree containing the "← Back to projects" `<button>` (outside `[role="tree"]`) plus an active-project label. Tree walk now operates on `filteredTree` instead of `tree`.
- projects/spec_driven/frontend/src/components/Home.tsx — repurposed as project picker. Signature changed to `{ tree, loadError, onPick }`; renders `.picker-view` with a centered `<ul className="picker-list">` of `<button className="picker-link">` rows. Click invokes `onPick({type, name})` which (in App.tsx) writes localStorage and navigates to `/project/{type}/{name}`. Drops the prior "Build regen prompt for the whole project" sub-text and "{N} top-level sections loaded" fallback — the picker is now a primary surface, not a secondary one. `discoverProjects` helper now returns `ActiveProject[]` (was a local `ProjectRef` type).
- projects/spec_driven/frontend/src/styles.css — new `.app-root.app-root-landing` rule (block layout, single column for landing). New `.picker-pane`, `.picker-view`, `.picker-header`, `.picker-list`, `.picker-item`, `.picker-link`, `.picker-type`, `.picker-sep`, `.picker-name` rules under "Picker (landing)". New `.sidebar-header`, `.sidebar-back`, `.sidebar-active-project` rules. The prior `.home-view` / `.home-projects` / `.home-project-list` / `.home-project-item` rules are dropped (they covered the old in-sidebar list mode that no longer exists). Light-theme palette honored (`--bg`, `--bg-panel`, `--bg-hover`, `--border`, `--text-muted`, `--accent`, `--border-strong`).
- projects/spec_driven/frontend/test/Sidebar.test.tsx — `renderSidebar` helper passes the new required `activeProject={{ type: "development", name: "spec_driven" }}` and `onBackToProjects={() => {}}` props. Existing assertions still pass: filtered tree contains the same `development/spec_driven` subtree the fixture defines, so all 56 vitest cases (5 files) still pass.

No conflicts found in: interview/qa.md, findings/dossier.md, validation/* (filtering is a UX concern; AC-3 wire-shape contract unchanged, AC-26 sidebar structural sanity still holds for the active project), projects/spec_driven/backend/* (no backend changes — `/api/tree`, path-sandbox, `EXPOSED_TREE` all unchanged), projects/spec_driven/frontend/src/components/{Reader,ProjectPage,StagePage,RegeneratePanel,QaView,Editor,Breadcrumb,BrokenLink,ParseFallback}.tsx (untouched — they don't depend on the picker / sidebar shape), projects/spec_driven/README.md (run targets unchanged).

Discovery (out of scope, not fixed): `tsc -b` in `projects/spec_driven/frontend/` errors at `src/components/ProjectPage.tsx:43` — `project_type: projectType as "ai_video" | "development"` is cast to a wider union than `ProjectDeleteRequest.project_type: "ai_video"` declares in `src/types.ts`. Follow-up 011's changelog states the type was widened but `types.ts` line 104 was not actually updated. This is a pre-existing typecheck blocker independent of follow-up 012; flagging for a future surgical fix to `types.ts`.

## Follow-up 011 — 2026-05-06 20:42:45
Source: user_input/follow_ups/011-20260506-204245-generic-delete-and-prod-mode-coverage.md
Summary: Two coupled fixes for follow-up 010. **(1) Prod-mode 405 regression + validation-rule patch.** User reported `DELETE /api/project` returning 405 in prod mode: the static-files mount at `/` (with `html=True`) catches non-GET requests for any path that doesn't match a route and returns 405 because StaticFiles only allows GET/HEAD; this masks missing-route bugs. The 010 unit tests used `serve_static=False`, which silently passed on the wrong outcome (404 from FastAPI no-route) and never exercised the static-mount-shadowing path. Closure: new `prod_client` fixture in `test_project_deleter.py` with `serve_static=True`; new `test_delete_endpoint_reachable_with_static_mount` (per-endpoint regression) + `test_all_state_changing_endpoints_unshadowed_by_static` (class-of-failure sweep iterating `GUARDED_ROUTES` × `serve_static=True` asserting none return 405). Validation rule patched in `agent_refs/validation/development.md` move #1: appended a clause requiring "every state-changing endpoint MUST have at least one pytest integration test with `serve_static=True`" with severity `blocker`, citation `spec_driven-011`. **(2) Generic delete across task types.** `ALLOWED_TASK_TYPES_FOR_DELETE` extended from `{"ai_video"}` to `{"ai_video", "development"}` via new `_TASK_TYPE_TO_OUTPUT_DIR` mapping (`{"ai_video": "ai_videos", "development": "projects"}`); `delete()` reads the mapping for the output dir instead of hardcoding `ai_videos/`; defense-in-depth allowed-top-level set broadened to `{"specs", "ai_videos", "projects"}`. New `SelfDeleteRefused` exception protects the running webapp itself (`development/spec_driven`) from UI-driven self-immolation → returns 400 with `kind=self_delete_refused`. New api.py exception handler chain entry. Frontend `types.ts` widens `ProjectDeleteRequest.project_type` from `"ai_video"` literal to `"ai_video" | "development"`. `ProjectPage.tsx` rewritten: removes the `projectType === "ai_video"` gate (danger zone now renders for any allowed type); resolves output-dir name client-side via mirror map for the confirm dialog labels (`projects/{name}/` for development, `ai_videos/{name}/` for ai_video); special-cases `development/spec_driven` with a "Refused" message instead of the trigger button. New tests: `test_self_delete_refused`, `test_delete_synthetic_development_project`, `test_delete_only_output_dir_when_spec_missing`, `test_delete_endpoint_self_delete_refused_400`, plus the prod-mode regression tests above. Total: 34 tests pass in `test_project_deleter.py`; full backend suite: 244 passed + 2 skipped.

## Follow-up 010 — 2026-05-06 20:29:46
Source: user_input/follow_ups/010-20260506-202946-parent-level-delete-ai-video.md
Summary: Parent-level project delete for `task_type=ai_video` only. New backend module `libs/project_deleter.py` with `ProjectDeleter` class (strict slug regex `^[a-zA-Z0-9_][a-zA-Z0-9_-]*$`, task-type guard rejecting non-ai_video with 400, dual-path target computation under `specs/ai_video/{name}/` + `ai_videos/{name}/`, defense-in-depth path-resolution check before any FS mutation, refusal to follow root-level symlinks, `shutil.rmtree`-equivalent recursive delete, structured `DeleteResult` with `deleted_paths` + `not_found_paths`); `emit_audit_event` helper appends a `project.deleted` line to `.audit/adhoc_agents/{date}/manual_ops/events.jsonl` (best-effort; doesn't fail the delete on audit-write error). New `DELETE /api/project` endpoint added to `api.py` with `ProjectDeleteBody` Pydantic model; `("DELETE", "/api/project")` added to `api_security.py::GUARDED_ROUTES` so foreign-Origin requests get 403. Frontend: new `ProjectDeleteRequest` + `ProjectDeleteResult` types in `types.ts`; new `deleteProject` API call in `api.ts`; `ProjectPage.tsx` rewritten with a "Danger zone" section that's gated on `projectType === "ai_video"` — two-step confirmation (click "Delete project" → reveal the two paths to be removed → require user to type the project name verbatim before "Permanently delete" enables); on success, brief result banner showing `deleted_paths` + any `not_found_paths` then auto-redirect to `/` with a tree refresh. Light-theme `danger-zone-*` CSS classes added to `styles.css` (red-bordered card for the trigger, error-bg confirmation panel, success tint-a result banner). New unit-test file `backend/tests/unit/test_project_deleter.py` (28 cases) covers slug accept/reject parametrized list, task-type guard, project-not-found, synthetic-project full delete, partial delete when output dir missing, plus 4 HTTP integration tests (Origin/Host 403, unsupported_task_type 400, invalid_project_name 400, not_found 404). Full backend test suite: 238 passed + 2 skipped (POSIX-only).
