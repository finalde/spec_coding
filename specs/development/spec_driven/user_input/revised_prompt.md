...
- **Backend:** FastAPI.
- **Frontend:** React.
- **Layout (amended by follow-up 014):** single solution at `projects/spec_driven/` with `apps/api/` (FastAPI), `apps/ui/` (React), and a shared `libs/{infrastructure,domain,application,common}/`. One solution-root `README.md`, `Makefile`, `pyproject.toml`. The previous `backend/` + `frontend/` split is renamed to `apps/api/` + `apps/ui/`.
