...
Last regenerated: 2026-05-03 16:48:40 — header bump for follow-up 009 (four coupled UX gaps: sidebar second section renamed `Projects` → `Specs` with FR-3 / FR-15 spec-literal patches; project node click navigates to `/project/{type}/{name}` and stage node click navigates to a new `/stage/{type}/{name}/{stage}` route + `StagePage`, with the disclosure caret split into its own toggle button so expansion is independent of navigation; pin-loader regex in `Reader.tsx` corrected to read `<!-- pin source=… id=… -->` (canonical promotions.py shape) so QA pin state finally hydrates from `promoted.md`; markdown `Renderer` extended with optional `PinContext` that injects 📌 toggles next to `**FR-NN.**` / `**NFR-NN.**` / `**AC-NN.**` / `**SYS-NN.**` / `**OQ-NN.**` markers in spec / validation / findings markdown — closing FR-35 coverage gap for non-QA stages). Prior follow-ups 008 (Specs subtree mirrors `specs/{type}/{name}/`; Home project list), 007 (Makefile `--host`/`--port` flag mismatch), 006 (Vite proxy `Origin` rewrite + class-of-failure into `agent_refs/validation/`), 005 (light-theme relocation), 004 (loopback-alias backend fix), 003 (run-backend / run-frontend split), 002 (friendly regen UI), 001 (editable webapp) preserved.
